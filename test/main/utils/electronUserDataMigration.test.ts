...
describe('electron userData migration', () => {
  ...
  it('reuses populated legacy userData by default instead of copying it during startup', () => {
    ...
    const currentPath = path.join(root, 'Ordis');
  ...
  it('prefers an already populated Ordis directory over older legacy data', () => {
    const root = createTempRoot();
    const completedNewPath = path.join(root, 'Ordis');
  ...
  it('uses populated Ordis when both current product-name and new package-name paths exist', () => {
    const root = createTempRoot();
    const completedNewPath = path.join(root, 'Ordis');
  ...
  it('reuses existing Ordis data when the current product name is Ordis', () => {
    const root = createTempRoot();
    const completedNewPath = path.join(root, 'Ordis');
    const currentProductPath = path.join(root, 'Ordis');
