...
describePosix('OpenCode packaged-runtime preflight integration', () => {
  ...
  it('keeps OpenCode launch preflight and bridge commands working when packaged Electron starts with an empty PATH', async () => {
    ...
    const providerEnv = await buildProviderAwareCliEnv({
      ...
      env: {
        ...
        CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY: '/mock/mcp-server/index.js',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON: '["/mock/mcp-server/index.js"]',
      },
    });
  ...
  it('resolves the Agent Teams MCP command to shell Node when GUI PATH is empty', async () => {
    ...
    expect(providerEnv.env.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).toBe(nodePath);
    expect(providerEnv.env.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).not.toBe('node');
    expect(providerEnv.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY).toBeTruthy();
    expect(providerEnv.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON).toContain(
      providerEnv.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY ?? ''
    );
  });
});
