...
describe('buildProviderAwareCliEnv', () => {
  ...
  it('builds shared env for generic CLI launches when no provider is specified', async () => {
    ...
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).toBe('node');
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY).toBe('/app/mcp-server/index.js');
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON).toBe(
  ...
  it('serializes Agent Teams MCP launch env overrides for OpenCode provider commands', async () => {
    ...
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON).toBe(
      '{"ELECTRON_RUN_AS_NODE":"1"}'
    );
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).toBe(
      '/opt/Ordis/Ordis'
    );
  });
  ...
  it('preserves explicit local Agent Teams MCP launch env for OpenCode provider commands', async () => {
    ...
    const result = await buildProviderAwareCliEnv({
      providerId: 'opencode',
      env: {
        CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND: 'custom-node',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY: '/custom/mcp.js',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON: '["/custom/mcp.js"]',
        ELECTRON_RUN_AS_NODE: '1',
      },
    });

    expect(resolveAgentTeamsMcpLaunchSpecMock).not.toHaveBeenCalled();
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).toBe('custom-node');
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY).toBe('/custom/mcp.js');
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON).toBe('["/custom/mcp.js"]');
    expect(result.env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON).toBe(
      '{"ELECTRON_RUN_AS_NODE":"1"}'
    );
    expect(result.env.ELECTRON_RUN_AS_NODE).toBeUndefined();
