...
export async function createOpenCodeLiveHarness(input: {
  ...
  const orchestratorCli =
    process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH?.trim() || DEFAULT_ORCHESTRATOR_CLI;
  ...
  const bridgeEnv: NodeJS.ProcessEnv = {
    ...
    CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND: mcpLaunchSpec.command,
    CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY: mcpLaunchSpec.args[0] ?? '',
    CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON: JSON.stringify(mcpLaunchSpec.args),
    CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON: JSON.stringify(mcpLaunchSpec.env ?? {}),
  };
