  ...
  it('resolves command env lazily for each bridge command', async () => {
    ...
    const client = createClient({
      envProvider: () => {
        ...
        return {
          PATH: '/usr/bin',
          CLAUDE_MULTIMODEL_ORDIS_MCP_URL: `http://127.0.0.1:${5000 + envVersion}/mcp`,
        };
      },
    });
    ...
    expect(runner.calls[0].env).toMatchObject({
      CLAUDE_MULTIMODEL_ORDIS_MCP_URL: 'http://127.0.0.1:5001/mcp',
      OPENCODE_DISABLE_AUTOUPDATE: '1',
    });
    expect(runner.calls[1].env).toMatchObject({
      CLAUDE_MULTIMODEL_ORDIS_MCP_URL: 'http://127.0.0.1:5002/mcp',
      OPENCODE_DISABLE_AUTOUPDATE: '1',
    });
  });
