...
const controlFile = process.env.ORDIS_MCP_TEST_CONTROL_FILE;
const healthIdentity =
  process.env.ORDIS_MCP_HTTP_IDENTITY_SERVICE === 'agent-teams-mcp-http' &&
  process.env.ORDIS_MCP_HTTP_CLAUDE_DIR_HASH &&
  process.env.ORDIS_MCP_HTTP_LAUNCH_SPEC_HASH &&
  process.env.ORDIS_MCP_HTTP_OWNER_INSTANCE_ID
    ? {
        schemaVersion: 1,
        ...
        port,
        endpoint,
        claudeDirHash: process.env.ORDIS_MCP_HTTP_CLAUDE_DIR_HASH,
        launchSpecHash: process.env.ORDIS_MCP_HTTP_LAUNCH_SPEC_HASH,
        ownerInstanceId: process.env.ORDIS_MCP_HTTP_OWNER_INSTANCE_ID,
      }
    : null;
...
async function main() {
  ...
  const mcpUrl = process.env.CLAUDE_MULTIMODEL_ORDIS_MCP_URL || null;
...
describePosix('AgentTeamsMcpHttpServer integration', () => {
  ...
  beforeEach(async () => {
    ...
    originalControlFileEnv = process.env.ORDIS_MCP_TEST_CONTROL_FILE;
  ...
  afterEach(async () => {
    ...
    if (originalControlFileEnv === undefined) {
      delete process.env.ORDIS_MCP_TEST_CONTROL_FILE;
    } else {
      process.env.ORDIS_MCP_TEST_CONTROL_FILE = originalControlFileEnv;
    }
  ...
  function createControlledServer(input: {
    ...
    process.env.ORDIS_MCP_TEST_CONTROL_FILE = input.controlFile;
    return server;
  }
  ...
  it('passes a refreshed healthy MCP URL into a real bridge child process after cached health goes stale', async () => {
    ...
    const client = new OpenCodeBridgeCommandClient({
      ...
      envProvider: async () => {
        ...
        return {
          ...bridgeEnv,
          CLAUDE_MULTIMODEL_ORDIS_MCP_URL: mcpHttpServer.url,
        };
      },
      requestIdFactory: () => {
        requestIdCounter += 1;
  ...
  it('fails closed when a bridge child receives an unreachable MCP URL without env refresh', async () => {
    ...
    const client = new OpenCodeBridgeCommandClient({
      ...
      env: {
        PATH: process.env.PATH,
        CLAUDE_MULTIMODEL_ORDIS_MCP_URL: first.url,
      },
