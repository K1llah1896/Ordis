...
describe('Team runtime memory safe e2e', () => {
  ...
  const cliSmokeIt =
    ...
    process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH?.trim() &&
    existsSync(process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH.trim())
      ? it
      : it.skip;
  ...
  cliSmokeIt('reports RSS for a real Anthropic teammate CLI process', async () => {
    ...
    const agentId = `${memberName}@${teamName}`;
    const projectPath = path.join(tempDir, 'project');
    const cliPath = process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH!.trim();
    await fs.mkdir(projectPath, { recursive: true });
    await writeTeamFixture({
      tempDir,
      teamName,
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          CLAUDE_CODE_EXPERIMENTAL_ORDIS: '1',
