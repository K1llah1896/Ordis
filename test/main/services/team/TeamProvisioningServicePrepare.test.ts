...
const REQUIRED_MOCK_ORDIS_TOOLS = [
...
function writeMockMcpServer(
  ...
  const tools = REQUIRED_MOCK_ORDIS_TOOLS.filter(
...
describe('TeamProvisioningService prepare/auth behavior', () => {
  ...
  it('adds member-work-sync turn-settled spool env for Codex provisioning', async () => {
    ...
    svc.setRuntimeTurnSettledEnvironmentProvider(async ({ provider }) =>
      provider === 'codex'
        ? { ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT: '/tmp/runtime-hooks' }
        : null
    );
    ...
    expect(result.env.ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT).toBe('/tmp/runtime-hooks');
  });
