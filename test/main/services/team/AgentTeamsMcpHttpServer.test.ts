...
describe('AgentTeamsMcpHttpServer', () => {
  ...
  it('starts the MCP server over HTTP with hidden app-owned process env', async () => {
    ...
    expect(spawnProcess).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        ORDIS_MCP_HTTP_HOST: '127.0.0.1',
        ORDIS_MCP_HTTP_PORT: '41001',
        ORDIS_MCP_HTTP_ENDPOINT: '/mcp',
      })
    );
  });
  ...
  it('cleans up a proven legacy orphan MCP HTTP process without live consumers', async () => {
    ...
    const details = `${command} ORDIS_MCP_CLAUDE_DIR=${getClaudeBasePath()} ORDIS_MCP_TRANSPORT=httpStream ORDIS_MCP_HTTP_HOST=127.0.0.1 ORDIS_MCP_HTTP_PORT=${orphanPort} ORDIS_MCP_HTTP_ENDPOINT=/mcp`;
    const server = new AgentTeamsMcpHttpServer({
      statePath,
      resolveLaunchSpec: async () => ({
        command: 'node',
  ...
  it('keeps a proven legacy orphan MCP HTTP process when live consumers still reference it', async () => {
    ...
    const orphanDetails = `${command} ORDIS_MCP_CLAUDE_DIR=${getClaudeBasePath()} ORDIS_MCP_TRANSPORT=httpStream ORDIS_MCP_HTTP_HOST=127.0.0.1 ORDIS_MCP_HTTP_PORT=${orphanPort} ORDIS_MCP_HTTP_ENDPOINT=/mcp`;
    ...
    const server = new AgentTeamsMcpHttpServer({
      ...
      readProcessDetails: async (pid) =>
        pid === 9002
          ? orphanDetails
          : `CLAUDE_MULTIMODEL_ORDIS_MCP_URL=${url}`,
      readProcessStartTimeMs: async () => 0,
      killProcess,
