...
import {
  ...
  REQUIRED_ORDIS_APP_TOOL_IDS,
  REQUIRED_ORDIS_APP_TOOLS,
  REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS,
  REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS,
  REQUIRED_ORDIS_RUNTIME_TOOLS,
...
describe('OpenCode MCP tool availability', () => {
  ...
  it('loads launch-visible teammate-operational tools from the controller catalog', () => {
    ...
    expect(REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS).toContain('task_start');
    expect(REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS).not.toContain('lead_briefing');
    expect(REQUIRED_ORDIS_APP_TOOLS).toEqual([
      ...REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS,
      ...REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS,
    ]);
    expect(REQUIRED_ORDIS_APP_TOOL_IDS).toContain('agent-teams_message_send');
    expect(REQUIRED_ORDIS_APP_TOOL_IDS).toContain('agent-teams_member_briefing');
    expect(REQUIRED_ORDIS_APP_TOOL_IDS).toContain('agent-teams_cross_team_send');
  });
  ...
  it('proves required tools through experimental tool ids', async () => {
    const client = fakeToolClient({
      ids: REQUIRED_ORDIS_RUNTIME_TOOLS.map((tool) =>
    ...
    await expect(
      probe.proveRequiredTools({
        serverName: 'agent-teams',
        requiredTools: [...REQUIRED_ORDIS_RUNTIME_TOOLS],
  ...
  it('falls back to provider/model tool definitions when ids route fails', async () => {
    const client = fakeToolClient({
      ...
      tools: REQUIRED_ORDIS_RUNTIME_TOOLS.map((tool) => ({
