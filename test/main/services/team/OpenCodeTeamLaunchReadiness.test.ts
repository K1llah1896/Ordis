...
import { REQUIRED_ORDIS_RUNTIME_TOOLS } from '../../../../src/main/services/team/opencode/mcp/OpenCodeMcpToolAvailability';
import {
...
function toolProof(overrides: Partial<OpenCodeMcpToolProof> = {}): OpenCodeMcpToolProof {
  return {
    ...
    canonicalExpectedIds: Object.fromEntries(
      REQUIRED_ORDIS_RUNTIME_TOOLS.map((tool) => [tool, `agent_teams_${tool}`])
    ),
    observedTools: REQUIRED_ORDIS_RUNTIME_TOOLS.map((tool) => `agent_teams_${tool}`),
    missingTools: [],
    matchedByRequiredTool: Object.fromEntries(
      REQUIRED_ORDIS_RUNTIME_TOOLS.map((tool) => [tool, `agent_teams_${tool}`])
    ),
    aliasMatchedByRequiredTool: Object.fromEntries(
      REQUIRED_ORDIS_RUNTIME_TOOLS.map((tool) => [tool, null])
    ),
    diagnostics: [],
    ...overrides,
  };
