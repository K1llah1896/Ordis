...
describe('OpenCodeMcpBridgeEnv', () => {
  ...
  it('detects complete local MCP launch env', () => {
    expect(
      hasOpenCodeLocalMcpLaunchEnv({
        CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND: 'node',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY: 'mcp-server/dist/index.js',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON: '["mcp-server/dist/index.js"]',
      })
    ).toBe(true);
    ...
    expect(
      hasOpenCodeLocalMcpLaunchEnv({
        CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND: 'node',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY: '',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON: '["mcp-server/dist/index.js"]',
      })
    ).toBe(false);
  });
  ...
  it('copies local MCP launch env for HTTP fallback without copying the HTTP URL', () => {
    const target: NodeJS.ProcessEnv = {
      CLAUDE_MULTIMODEL_ORDIS_MCP_URL: 'http://127.0.0.1:41001/mcp',
    };
    ...
    copyOpenCodeLocalMcpLaunchEnv(
      {
        ...
        CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY: 'mcp-server/dist/index.js',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON: '["mcp-server/dist/index.js"]',
        CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON: '{"ELECTRON_RUN_AS_NODE":"1"}',
      },
      target
    );

    expect(target.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).toBe('node');
    expect(target.CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY).toBe('mcp-server/dist/index.js');
    expect(target.CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON).toBe('["mcp-server/dist/index.js"]');
    expect(target.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON).toBe(
      '{"ELECTRON_RUN_AS_NODE":"1"}'
    );
    expect(target.CLAUDE_MULTIMODEL_ORDIS_MCP_URL).toBe('http://127.0.0.1:41001/mcp');
  });
  ...
  it('snapshots explicit local MCP launch env before mutating an env object', () => {
    ...
    expect(env.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).toBeUndefined();
    expect(env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON).toBeUndefined();
  });
  ...
  it('migrates legacy MCP child env into the local MCP env JSON snapshot', () => {
    ...
    expect(snapshot?.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON).toBe(
      '{"ELECTRON_RUN_AS_NODE":"1"}'
    );
  ...
  it('removes local MCP launch env when explicitly requested', () => {
    ...
    expect(env.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND).toBeUndefined();
    expect(env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY).toBeUndefined();
    expect(env.CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON).toBeUndefined();
    expect(env.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON).toBeUndefined();
    expect(env.ELECTRON_RUN_AS_NODE).toBeUndefined();
    expect(env.CLAUDE_MULTIMODEL_ORDIS_MCP_URL).toBe('http://127.0.0.1:41001/mcp');
  });
});
