...
liveDescribe('OpenCode accept-fast delivery live e2e', () => {
  ...
  beforeEach(async () => {
    ...
    previousOrchestratorCliPath = process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH;
    process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH =
      previousOrchestratorCliPath?.trim() || DEFAULT_ORCHESTRATOR_CLI;
    previousMcpClaudeDir = process.env.ORDIS_MCP_CLAUDE_DIR;
    process.env.ORDIS_MCP_CLAUDE_DIR = tempClaudeRoot;
    harness = null;
    teamName = null;
  });
  ...
  afterEach(async () => {
    ...
    if (previousOrchestratorCliPath === undefined) {
      delete process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH;
    } else {
      process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH = previousOrchestratorCliPath;
    }
    if (previousMcpClaudeDir === undefined) {
      delete process.env.ORDIS_MCP_CLAUDE_DIR;
    } else {
      process.env.ORDIS_MCP_CLAUDE_DIR = previousMcpClaudeDir;
    }
