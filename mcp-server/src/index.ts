  ...
  return {
    ...
    httpStream: {
      host:
        ...
        env.ORDIS_MCP_HTTP_HOST?.trim() ??
        DEFAULT_HTTP_HOST,
      port: parsePort(getArgValue(argv, '--port') ?? env.ORDIS_MCP_HTTP_PORT),
      endpoint: normalizeEndpoint(
        getArgValue(argv, '--endpoint') ?? env.ORDIS_MCP_HTTP_ENDPOINT
      ),
    },
  };
}
