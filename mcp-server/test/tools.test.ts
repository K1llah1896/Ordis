...
import { ORDIS_REGISTERED_TOOL_NAMES, registerTools } from '../src/tools';
...
describe('agent-teams-mcp tools', () => {
  ...
  it('registers the full expected MCP tool surface', () => {
    expect([...tools.keys()].sort()).toEqual([...ORDIS_REGISTERED_TOOL_NAMES].sort());
  });
  ...
  it('uses forced app claude dir over model-supplied claudeDir when configured', async () => {
    ...
    const previousForcedDir = process.env.ORDIS_MCP_CLAUDE_DIR;
    process.env.ORDIS_MCP_CLAUDE_DIR = forcedClaudeDir;
    try {
      const sent = parseJsonToolResult(
        await getTool('message_send').execute({
      ...
      if (previousForcedDir === undefined) {
        ...
        process.env.ORDIS_MCP_CLAUDE_DIR = previousForcedDir;
      }
    }
  });
