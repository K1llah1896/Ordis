...
- For this workspace, the canonical GitHub repository is `777genius/Ordis`.
- When reviewing or discussing PR `#126`, inspect `777genius/Ordis#126` unless the user explicitly names another repository.
...
- Local live/prove scripts should use `scripts/lib/live-smoke-runtime.mjs`, which defaults to `cli-source` unless `CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH` is explicitly set.
...
- Use the built wrapper only for release or production-like smoke checks. Build first in `/Users/belief/dev/projects/claude/agent_teams_orchestrator` with `bun run build`, then set `CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH=/Users/belief/dev/projects/claude/agent_teams_orchestrator/cli`.
