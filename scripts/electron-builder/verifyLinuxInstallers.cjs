...
function verifyDeb(debPath) {
  ...
  assertContains(dataList, './usr/bin/Ordis\n', path.basename(debPath));
  assertContains(dataList, './opt/Ordis/Ordis\n', path.basename(debPath));
  assertContains(dataList, './opt/Ordis/chrome-sandbox\n', path.basename(debPath));

  const launcherLine = dataVerboseList
    .split(/\r?\n/)
    .find((line) => line.endsWith('./usr/bin/Ordis'));
  if (!launcherLine || !launcherLine.startsWith('-rwx')) {
    fail(`/usr/bin/Ordis is not executable in ${debPath}`);
  }

  const launcher = extractDebTarFile(debPath, dataMember, './usr/bin/Ordis');
  assertContains(launcher, '/opt/Ordis/Ordis', 'CLI launcher');
  ...
  assertContains(postinst, 'SANDBOX_PATH="/opt/Ordis/chrome-sandbox"', 'deb postinst');
