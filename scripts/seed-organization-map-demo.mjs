...
function getAppDataBasePath() {
  if (process.env.ORDIS_ELECTRON_USER_DATA_DIR) {
    return process.env.ORDIS_ELECTRON_USER_DATA_DIR;
  }
  if (process.env.ORDIS_APP_DATA_DIR) {
    return process.env.ORDIS_APP_DATA_DIR;
  }
  if (process.platform === 'darwin') {
    return path.join(os.homedir(), 'Library', 'Application Support', 'Ordis');
  }
...
function getClaudeRootPath() {
  const override = process.env.ORDIS_ELECTRON_CLAUDE_ROOT?.trim();
