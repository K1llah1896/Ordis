...
const explicitRuntimeCliPath = process.env.CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH?.trim() ?? '';
...
const organizationDemoEnv = 'ORDIS_ORG_DEMO';
  ...
  if (!response.ok || !response.body) {
    throw new Error(
      formatGitHubReleaseDownloadError({
        ...
        notFoundHint:
          '404 usually means the runtime release or asset is missing, private, or inaccessible. If the orchestrator repository is private, run gh auth login with access to it or set CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH to a local runtime.',
        ...download,
      })
    );
  }
...
async function resolveRuntimeCli() {
  if (explicitRuntimeCliPath) {
    if (!isExecutable(explicitRuntimeCliPath)) {
      throw new Error(
        `CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH is not executable: ${explicitRuntimeCliPath}`
      );
    }
...
async function main() {
  ...
  const uiEnv = {
    ...
    CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH: resolvedRuntime.binaryPath,
  };
