...
export const RUNTIME_TURN_SETTLED_SPOOL_ROOT_ENV =
  'ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT';
```
...
- add a composition test or safe integration test proving `ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT` is present in the env passed to `OpenCodeBridgeCommandClient`;
...
4. **Composition order.** Write a test or narrow integration seam proving OpenCode bridge env receives `ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT` before `OpenCodeBridgeCommandClient` construction.
...
- OpenCode bridge process receives `ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT`.
