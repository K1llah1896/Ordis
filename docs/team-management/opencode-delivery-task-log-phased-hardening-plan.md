...
- workspace trust env filtering must preserve the `ORDIS_RUNTIME_TURN_SETTLED_` prefix;
...
- Codex primary and secondary launches include `ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT`;
...
81. Add workspace trust/preflight tests proving `ORDIS_RUNTIME_TURN_SETTLED_SPOOL_ROOT` survives allowed env filtering.
