...
- `claude_team` already exports `ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES` from `agent-teams-controller`; app-side required tools should derive from that instead of duplicating a second list.
...
export function canonicalizeAgentTeamsToolName(rawName: string): string {
  ...
  for (const prefix of ORDIS_PREFIXES) {
...
export function isAgentTeamsToolUse(input: {
  ...
  const hasKnownPrefix =
    rawName !== canonical || ORDIS_PREFIXES.some((prefix) => rawName.startsWith(prefix));
  if (hasKnownPrefix) {
    return true;
  }
...
console.log(catalog.ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES.join('\n'))
NODE
```
...
```ts
const REQUIRED_ORDIS_RUNTIME_TOOLS = [
...
```ts
const REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS = [
  'runtime_bootstrap_checkin',
  'runtime_deliver_message',
  'runtime_task_event',
  'runtime_heartbeat',
...
const REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS = [
  'member_briefing',
  'task_add_comment',
...
const REQUIRED_ORDIS_DIRECT_MCP_TOOL_NAMES = [
  ...REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS,
  ...REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS,
] as const;
```
...
function matchAppMcpTools(observedDirectToolNames: string[], route: string): AppMcpToolProof {
  ...
  const missingTools = REQUIRED_ORDIS_DIRECT_MCP_TOOL_NAMES.filter(
...
- Public readiness/evidence still exposes canonical ids like `agent-teams_message_send`, not plain direct names, so production evidence remains comparable to `REQUIRED_ORDIS_APP_TOOL_IDS`.
...
const ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES =
  agentTeamsControllerModule.ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES;

export const REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS = [
  'runtime_bootstrap_checkin',
  'runtime_deliver_message',
  'runtime_task_event',
  'runtime_heartbeat',
...
export const REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS: readonly string[] = [
  ...ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES,
] as const;

export const REQUIRED_ORDIS_APP_TOOLS: readonly string[] = [
  ...REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS,
  ...REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS,
] as const;

export const REQUIRED_ORDIS_APP_TOOL_IDS = REQUIRED_ORDIS_APP_TOOLS.map((tool) =>
...
- `REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS` remains a literal tuple because runtime schema contracts depend on exact names.

Add a small import-shape test:

```ts
it('loads teammate-operational tool names from agent-teams-controller package main', () => {
  expect(REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS).toContain('message_send');
  expect(REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS).toContain('cross_team_send');
  expect(REQUIRED_ORDIS_TEAMMATE_OPERATIONAL_TOOLS).not.toContain('lead_briefing');
});
```
...
- Existing callers that truly mean runtime schema tools should use `REQUIRED_ORDIS_RUNTIME_PROOF_TOOLS`, not the full app list.
- Existing callers that mean launch-visible app tools should use `REQUIRED_ORDIS_APP_TOOLS` or `REQUIRED_ORDIS_APP_TOOL_IDS`.
...
it('uses full app tool ids for OpenCode readiness expectations', async () => {
  ...
  expect(result.evidence.observedMcpTools).toEqual(
    expect.arrayContaining(REQUIRED_ORDIS_APP_TOOL_IDS)
  );
});
```
...
it('keeps runtime schema validation scoped to runtime proof tools', () => {
  ...
  expect(REQUIRED_ORDIS_APP_TOOLS).toContain('message_send');
