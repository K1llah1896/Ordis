...
- это самый дешёвый способ гарантировать, что lead surface не попадёт в `ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES`
...
- не переиспользовать `ORDIS_NAMESPACED_TEAMMATE_OPERATIONAL_TOOL_NAMES` как proxy для lead-first surfaces
...
**3. Вычислять список динамически из `ORDIS_MCP_TOOL_GROUPS` по эвристике**
...
- `ORDIS_MCP_TOOL_GROUPS` определяет teammate visibility на уровне whole group через `teammateOperational`
- `task` group сейчас teammate-operational
- `lead_briefing`, если просто добавить его в `ORDIS_TASK_TOOL_NAMES`, автоматически попадёт в `ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES`
...
- `TeamProvisioningService` в lead bootstrap spec кладёт `permissionSeedTools` из `ORDIS_NAMESPACED_TEAMMATE_OPERATIONAL_TOOL_NAMES`
...
- `mcpToolCatalog.js` экспортирует `ORDIS_NAMESPACED_TEAMMATE_OPERATIONAL_TOOL_NAMES`
...
- catalog placement:
  ...
  - `teammateOperational: false`
  - `lead_briefing` не должен попадать в `ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES`
...
- `ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES` отражает новый teammate access policy
- `lead_briefing` явно отсутствует в `ORDIS_TEAMMATE_OPERATIONAL_TOOL_NAMES`
