...
export const ORDIS_ELECTRON_USER_DATA_DIR_ENV = 'ORDIS_ELECTRON_USER_DATA_DIR';
export const ORDIS_ELECTRON_CLAUDE_ROOT_ENV = 'ORDIS_ELECTRON_CLAUDE_ROOT';
...
export function resolveElectronDevClaudeRootOverride(
  ...
  return normalizeAbsolutePath(env[ORDIS_ELECTRON_CLAUDE_ROOT_ENV]);
}
...
export function applyElectronDevPathOverrides(
  ...
  const userDataDir = normalizeAbsolutePath(env[ORDIS_ELECTRON_USER_DATA_DIR_ENV]);
  const rawUserDataDir = env[ORDIS_ELECTRON_USER_DATA_DIR_ENV]?.trim();
  if (rawUserDataDir && !userDataDir) {
    warnings.push(`${ORDIS_ELECTRON_USER_DATA_DIR_ENV} must be an absolute path.`);
  }

  const appliedUserDataDir =
    userDataDir && ensureDirectory(userDataDir, warnings, ORDIS_ELECTRON_USER_DATA_DIR_ENV)
      ? userDataDir
      : null;
  ...
  const rawClaudeRoot = env[ORDIS_ELECTRON_CLAUDE_ROOT_ENV]?.trim();
  if (rawClaudeRoot && !claudeRoot) {
    warnings.push(`${ORDIS_ELECTRON_CLAUDE_ROOT_ENV} must be an absolute path.`);
  }

  const appliedClaudeRoot =
    claudeRoot && ensureDirectory(claudeRoot, warnings, ORDIS_ELECTRON_CLAUDE_ROOT_ENV)
      ? claudeRoot
      : null;
