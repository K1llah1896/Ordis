...
import {
  ORDIS_ELECTRON_CLAUDE_ROOT_ENV,
  ORDIS_ELECTRON_USER_DATA_DIR_ENV,
...
describe('applyElectronDevPathOverrides', () => {
  ...
  it('applies absolute userData and Claude root overrides before Electron storage migration', () => {
    ...
    const result = applyElectronDevPathOverrides(
      ...
      {
        [ORDIS_ELECTRON_USER_DATA_DIR_ENV]: userDataDir,
        [ORDIS_ELECTRON_CLAUDE_ROOT_ENV]: claudeRoot,
      }
    );
  ...
  it('rejects relative override paths', () => {
    ...
    const result = applyElectronDevPathOverrides(
      ...
      {
        [ORDIS_ELECTRON_USER_DATA_DIR_ENV]: 'relative-user-data',
        [ORDIS_ELECTRON_CLAUDE_ROOT_ENV]: 'relative-claude-root',
      }
    );
    ...
    expect(result.warnings).toEqual([
      `${ORDIS_ELECTRON_USER_DATA_DIR_ENV} must be an absolute path.`,
      `${ORDIS_ELECTRON_CLAUDE_ROOT_ENV} must be an absolute path.`,
    ]);
    expect(setPathCalls).toEqual([]);
  });
});
