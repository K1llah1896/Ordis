/**
 * Main process entry point for Ordis.
 *
 * Responsibilities:
...
if (process.env.ORDIS_DISABLE_GPU?.trim() === '1') {
  app.disableHardwareAcceleration();
  logger.info('Hardware acceleration disabled by ORDIS_DISABLE_GPU=1');
}
...
async function createOpenCodeRuntimeAdapterRegistry(
  ...
  bridgeEnv.ORDIS_MCP_CLAUDE_DIR = getClaudeBasePath();
  ...
  delete bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL;
  const applyMcpLaunchSpecEnv = async (
    ...
    try {
      ...
      if (mcpEntry) {
        targetEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND = mcpLaunchSpec.command;
        targetEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY = mcpEntry;
        targetEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON = JSON.stringify(mcpLaunchSpec.args);
        targetEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON = JSON.stringify(
          mcpLaunchSpec.env ?? {}
        );
      }
    } catch (error) {
  ...
  if (useHttpMcpBridge) {
    try {
      ...
      bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL = mcpHttpServer.url;
      bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL_HASH = mcpHttpServer.urlHash;
  ...
  if (
    shouldEnsureOpenCodeLocalMcpLaunchEnv({
      ...
      mcpUrl: bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL,
    })
  ) {
  ...
  const resolveBridgeCommandEnv = async (): Promise<NodeJS.ProcessEnv> => {
    ...
    try {
      ...
      bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL = mcpHttpServer.url;
      bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL_HASH = mcpHttpServer.urlHash;
      nextEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL = mcpHttpServer.url;
      nextEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL_HASH = mcpHttpServer.urlHash;
      await ensureOpenCodeLocalMcpLaunchEnv(nextEnv);
    } catch (error) {
      delete bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL;
      delete bridgeEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL_HASH;
      delete nextEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL;
      delete nextEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_URL_HASH;
      await ensureOpenCodeLocalMcpLaunchEnv(nextEnv);
      logger.warn(
...
async function initializeServices(): Promise<void> {
  ...
  tokenUsageFeature = createTokenUsageFeature({
    ...
    ccusageJsonPath: process.env.ORDIS_TOKEN_USAGE_CCUSAGE_JSON,
    tokscaleJsonPath: process.env.ORDIS_TOKEN_USAGE_TOKSCALE_JSON,
    ccusageCommand: readOptionalEnv('ORDIS_TOKEN_USAGE_CCUSAGE_COMMAND'),
    ccusageArgs: readOptionalEnvArgs('ORDIS_TOKEN_USAGE_CCUSAGE_ARGS'),
    tokscaleCommand: readOptionalEnv('ORDIS_TOKEN_USAGE_TOKSCALE_COMMAND'),
    tokscaleArgs: readOptionalEnvArgs('ORDIS_TOKEN_USAGE_TOKSCALE_ARGS'),
    commandImporterRefreshIntervalMs: readOptionalEnvNumber(
      'ORDIS_TOKEN_USAGE_COMMAND_REFRESH_MS'
    ),
