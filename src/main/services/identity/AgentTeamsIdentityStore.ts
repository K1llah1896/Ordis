...
export const ORDIS_IDENTITY_STORE_PATH_ENV = 'ORDIS_IDENTITY_STORE_PATH';
export const ORDIS_IDENTITY_SCHEMA_VERSION = 1;
...
export interface AgentTeamsIdentityStoreV1 {
  schemaVersion: typeof ORDIS_IDENTITY_SCHEMA_VERSION;
  clientId: string;
...
export function applyAgentTeamsIdentityEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const existing = env[ORDIS_IDENTITY_STORE_PATH_ENV];
  if (!isNonEmptyString(existing)) {
    env[ORDIS_IDENTITY_STORE_PATH_ENV] = getAgentTeamsIdentityStorePath();
  }
  return env;
}
...
function normalizeStoreRecord(value: unknown): AgentTeamsIdentityStoreV1 | null {
  ...
  if (value.schemaVersion !== ORDIS_IDENTITY_SCHEMA_VERSION) {
    return null;
  }
  ...
  return {
    schemaVersion: ORDIS_IDENTITY_SCHEMA_VERSION,
