...
const LOCAL_MCP_LAUNCH_ENV_KEYS = [
  'CLAUDE_MULTIMODEL_ORDIS_MCP_COMMAND',
  'CLAUDE_MULTIMODEL_ORDIS_MCP_ENTRY',
  'CLAUDE_MULTIMODEL_ORDIS_MCP_ARGS_JSON',
] as const;
const OPTIONAL_LOCAL_MCP_LAUNCH_ENV_KEYS = ['CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON'] as const;
...
export function copyOpenCodeLocalMcpLaunchEnv(
  ...
  if (!targetEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON?.trim()) {
    const legacyEnvJson = buildLegacyLocalMcpEnvJson(sourceEnv);
    if (legacyEnvJson) {
      targetEnv.CLAUDE_MULTIMODEL_ORDIS_MCP_ENV_JSON = legacyEnvJson;
    }
  }
}
