...
function buildMissingCliError(): Error {
  if (getConfiguredCliFlavor() === 'agent_teams_orchestrator') {
    return new Error(
      'Multimodel runtime not found. The packaged app must include resources/runtime/claude-multimodel, or development must provide CLAUDE_ORDIS_ORCHESTRATOR_CLI_PATH.'
    );
  }
    ...
    if (suggestions.length === 0) {
      ...
      if (!projectCwd) {
        ...
        for (const suggestion of suggestions) {
          ...
          if (
            toolNames.some((name) =>
              ORDIS_NAMESPACED_TEAMMATE_OPERATIONAL_TOOL_NAMES.includes(name)
            )
          ) {
