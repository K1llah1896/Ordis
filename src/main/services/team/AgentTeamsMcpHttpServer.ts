...
const MCP_HTTP_IDENTITY_SERVICE_ENV = 'ORDIS_MCP_HTTP_IDENTITY_SERVICE';
const MCP_HTTP_CLAUDE_DIR_HASH_ENV = 'ORDIS_MCP_HTTP_CLAUDE_DIR_HASH';
const MCP_HTTP_LAUNCH_SPEC_HASH_ENV = 'ORDIS_MCP_HTTP_LAUNCH_SPEC_HASH';
const MCP_HTTP_OWNER_INSTANCE_ID_ENV = 'ORDIS_MCP_HTTP_OWNER_INSTANCE_ID';
...
function hasManagedMcpDetails(details: string, port: number): boolean {
  return (
    processDetailsIncludeMarker(details, 'ORDIS_MCP_TRANSPORT=httpStream') &&
    processDetailsIncludeMarker(details, `ORDIS_MCP_HTTP_HOST=${MCP_HTTP_HOST}`) &&
    processDetailsIncludeMarker(details, `ORDIS_MCP_HTTP_PORT=${port}`) &&
    processDetailsIncludeMarker(details, `ORDIS_MCP_HTTP_ENDPOINT=${MCP_HTTP_ENDPOINT}`) &&
    processDetailsIncludeMarker(details, `ORDIS_MCP_CLAUDE_DIR=${getClaudeBasePath()}`)
  );
}
...
export class AgentTeamsMcpHttpServer {
  ...
  private async startOnceUnlocked(
    ...
    const childEnv = applyAgentTeamsIdentityEnv({
      ...process.env,
      ...launchSpec.env,
      ORDIS_MCP_CLAUDE_DIR: getClaudeBasePath(),
      ORDIS_MCP_TRANSPORT: 'httpStream',
      ORDIS_MCP_HTTP_HOST: MCP_HTTP_HOST,
      ORDIS_MCP_HTTP_PORT: String(port),
      ORDIS_MCP_HTTP_ENDPOINT: MCP_HTTP_ENDPOINT,
  ...
  private async hasLiveMcpConsumers(
    ...
    for (const row of rows) {
      ...
      if (
        processDetailsIncludeMarker(details, `CLAUDE_MULTIMODEL_ORDIS_MCP_URL=${url}`) ||
        processDetailsIncludeMarker(
          details,
          `CLAUDE_MULTIMODEL_ORDIS_MCP_URL_HASH=${urlHash}`
        )
      ) {
        return true;
      }
