...
export function getExpectedReleaseAssetUrl(
  ...
  switch (platform) {
    case 'darwin':
      return arch === 'arm64'
        ? `${base}/Ordis-${version}-arm64.dmg`
        : `${base}/Ordis-${version}-x64.dmg`;
    case 'win32':
      return `${base}/Ordis.Setup.${version}.exe`;
    case 'linux':
      return `${base}/Ordis-${version}.AppImage`;
    default:
      return null;
  }
}
...
export function getExpectedLatestMacArtifacts(
  ...
  return arch === 'arm64'
    ? [`Ordis-${version}-arm64-mac.zip`, `Ordis-${version}-arm64.dmg`]
    : [`Ordis-${version}-x64-mac.zip`, `Ordis-${version}-x64.dmg`];
}
